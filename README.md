...
- Reference: https://github.com/theniceboy/.config

```shell
ln -s [path]/dotfiles/tmux ~/.config/tmux
find ~/.config/tmux/scripts/ -type f -name "*.sh" -exec chmod u+x {} +
find ~/.config/tmux/tmux-status/ -type f -name "*.sh" -exec chmod u+x {} +
chmod u+x ~/.config/tmux/fzf_panes.tmux
